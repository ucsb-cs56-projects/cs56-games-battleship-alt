...
public class Battleship extends JFrame {
	...
	public static FilledShip [] filledShip = new FilledShip[5];	//Used to repaint the ship
	...
	public JPanel shipinput() {
		...
		input = new JPanel();
		mbar.setText("Choose ship's position and direction.");
	...
	private class GameListener implements ActionListener {
		public void actionPerformed(ActionEvent e) {
			...
			if (q == 0) {
				...
				try {
					if (gametype == pvp) {
						...
						try {
							...
							System.out
									.println("Could not listen on port: 22222");
						...
						try {
							...
							// TODO
							throw new PortFailedException("Failed to conect");
						}
				...
				} catch (PortFailedException e1) {
					JOptionPane.showMessageDialog(null, "No one "
							+ " so hard now, was it?", "Host Failed!",
							JOptionPane.INFORMATION_MESSAGE);
				...
				if (gametype == cvc)// Computer vs Computer
				{
					...
					boardPanel.add(autoBoard(enemy, you), BorderLayout.EAST);
					whoGoesFirst();
				} else if (gametype == pvpc)// Connect to Host
				{
					Player player2 = new Player("client");

					System.out
							.println("ATTEMPT TO CONNECT TO WHICH IP/HOSTNAME?");
					String connectTo = null;
					try {
						connectTo = "127.0.0.1";
					} catch (Exception ioe) {
						System.err
								.println("IO error trying to read your selection!"
										+ ioe.getMessage());
						System.exit(1);
					}
					...
					try {
						...
						System.out
								.println("IOException when connecting to host.\n"
										+ e1.getMessage());
						System.exit(-1);
					}
